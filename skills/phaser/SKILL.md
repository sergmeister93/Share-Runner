...
- **Title screen by default** — include a title/menu scene as the entry point. Boot directly into gameplay only if the user explicitly asks for it
- **In-game score HUD** — create a UIScene or HUD overlay that displays the player's score (and any lives, timer, or level indicator) on screen during gameplay
- Use parallel scenes for UI overlays (pause menu) only when requested

### Screen Margins

Keep UI and gameplay clear of the extreme screen edges so nothing is clipped on mobile devices or behind browser chrome. Define simple margin constants in `Constants.js` (e.g., a top and bottom inset as a small percentage of canvas height, such as `GAME.HEIGHT * 0.08`).

**Rules:**
- All UI text, buttons, and HUD elements should sit within the safe margins, not flush against the top or bottom edge
- Gameplay entities should not spawn in the extreme edge margins where they might be clipped
- The game-over screen, score panels, and restart buttons should offset from both the top and bottom margins
- Use `const usableH = GAME.HEIGHT - MARGIN.TOP - MARGIN.BOTTOM` for calculating proportional positions in UI scenes
- Game canvas and backgrounds should fill the full viewport
- Touch controls at the bottom must account for `MARGIN.BOTTOM`
...
const safeBottom = MARGIN.BOTTOM;
...
```js
// Simple fixed margins as a fraction of canvas height — no external dependency.
export const MARGIN = {
  TOP: GAME.HEIGHT * 0.08,
  BOTTOM: GAME.HEIGHT * 0.08,
  LEFT: 0,
  RIGHT: 0,
};
```
